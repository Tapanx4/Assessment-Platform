...
def login(login_data: schemas.UserLogin, request: Request, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        user_cache_key = f"user_row:{login_data.email}"
        cached_user = cache_service.get(user_cache_key)
        ...
        if cached_user:
            ...
            if user_row_obj:
                user_row = dict(user_row_obj)
                cache_service.set(user_cache_key, user_row, ttl=60)
            else:
                user_row = None
                
            t_db = time.perf_counter() - t_db_start
            
        metrics_manager.set_gauge("login_user_lookup", t_db)

        # 2. Bcrypt checkpw
        t_bcrypt_start = time.perf_counter()
        is_password_valid = False
        if user_row:
            is_password_valid = auth.verify_password(login_data.password, user_row["password_hash"])
        t_bcrypt = time.perf_counter() - t_bcrypt_start
        metrics_manager.set_gauge("login_bcrypt_checkpw", t_bcrypt)

        if not user_row or not is_password_valid:
        ...
        t_jwt = time.perf_counter() - t_jwt_start
        metrics_manager.set_gauge("login_jwt_creation", t_jwt)

        # 4. Refresh token insert & 5. Session commit
        ...
        t_insert = time.perf_counter() - t_insert_start - t_commit
        
        metrics_manager.set_gauge("login_refresh_token_insert", t_insert)
        metrics_manager.set_gauge("login_session_commit", t_commit)
        ...
        logger.info(
            "login took %.3f (lookup: %.3f, bcrypt: %.3f, jwt: %.3f, insert: %.3f, commit: %.3f)",
            t_total,
            t_db,
            t_bcrypt,
            t_jwt,
