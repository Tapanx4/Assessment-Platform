...
bg_executor = ThreadPoolExecutor(max_workers=30, thread_name_prefix="bg_tasks_worker")
...
def _log_login_audit_sync(user_id: Optional[int], email: str, action: str, details: str, ip_address: Optional[str], user_agent: Optional[str]):
    try:
...
async def _log_login_audit_task(user_id: Optional[int], email: str, action: str, details: str, ip_address: Optional[str], user_agent: Optional[str]):
    loop = asyncio.get_running_loop()
    await loop.run_in_executor(bg_executor, _log_login_audit_sync, user_id, email, action, details, ip_address, user_agent)

def _log_violation_event_sync(attempt_id: int, violation_type: str, severity: str, details: str, screenshot_path: Optional[str] = None):
    try:
        with BgSessionLocal() as db:
            try:
...
async def _log_violation_event_task(attempt_id: int, violation_type: str, severity: str, details: str, screenshot_path: Optional[str] = None):
    loop = asyncio.get_running_loop()
    await loop.run_in_executor(bg_executor, _log_violation_event_sync, attempt_id, violation_type, severity, details, screenshot_path)

def _log_screenshot_violation_sync(attempt_id: int, violation_type: str, severity: str, details: str, screenshot_path: str):
...
async def _log_screenshot_violation_task(attempt_id: int, violation_type: str, severity: str, details: str, screenshot_path: str):
    loop = asyncio.get_running_loop()
    await loop.run_in_executor(bg_executor, _log_screenshot_violation_sync, attempt_id, violation_type, severity, details, screenshot_path)
